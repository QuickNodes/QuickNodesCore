...
// Copyright (c) 2017-2018 The quicknodes developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zquicknodescontroldialog.h"
#include "ui_zquicknodescontroldialog.h"
...
std::list<std::string> ZquicknodesControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZquicknodesControlDialog::listMints;

ZquicknodesControlDialog::ZquicknodesControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZquicknodesControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZquicknodesControlDialog::~ZquicknodesControlDialog()
{
    delete ui;
}

void ZquicknodesControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZquicknodesControlDialog::updateList()
{
...
void ZquicknodesControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZquicknodesControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZquicknodes_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZquicknodesControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZquicknodesControlDialog::GetSelectedMints()
{
...
void ZquicknodesControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
