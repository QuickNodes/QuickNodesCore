...
#include "zquicknodescontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zqns ought to be enough for anybody." - Bill Gates, 2017
    ui->zqnspayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzqnsSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzqns->setEnabled(false);
        ui->pushButtonMintzqns->setToolTip(tr("zqns is currently disabled due to maintenance."));

        ui->pushButtonSpendzqns->setEnabled(false);
        ui->pushButtonSpendzqns->setToolTip(tr("zqns is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zqnspayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzqns_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zqns is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zqns...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zqns in ") +
...
void PrivacyDialog::on_pushButtonSpendzqns_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zqns is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zqns
        sendzqns();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zqns
    sendzqns();
}

void PrivacyDialog::on_pushButtonZquicknodesControl_clicked()
{
    ZquicknodesControlDialog* zqnsControl = new ZquicknodesControlDialog(this);
    zqnsControl->setModel(walletModel);
    zqnsControl->exec();
}

void PrivacyDialog::setZquicknodesControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzqnsSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzqns()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid quicknodes Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zqnspayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zqns is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " quicknodes </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zqns</b>";
    ...
    // use mints from zqns selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZquicknodesControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZquicknodesControlDialog::GetSelectedMints();
    }
    ...
    // Clear zquicknodes selector in case it was used
    ZquicknodesControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zqns Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " quicknodes, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zqns Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zqns </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zqns "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zqns "));
    ui->labelzqnsAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzqnsSyncStatus->setVisible(fShow);
}
