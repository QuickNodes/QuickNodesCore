...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    Object zquicknodesObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zquicknodesObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zquicknodesObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zqnssupply", zquicknodesObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"quicknodesaddress\"\n"
...
#ifdef ENABLE_WALLET
        // Case 1: quicknodes address and we have full public key:
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"quicknodesaddress\" \"signature\" \"message\"\n"
